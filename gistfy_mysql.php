...
class gistfy {
  ...
  public function gettag($mytoken) {
    ...
    $statement = $con->prepare("SELECT tag FROM lexicon WHERE `token` =  ?");
//     $token = 'coffee';
    $statement->bind_param("s", $mytoken);
    $rowtag = '';
    $statement->bind_result($rowtag);
    $statement->execute();
    while ($statement->fetch()){
      $tags = explode(", ", str_replace(array("[", "]", '"', '"', '\n'), "", $rowtag));
    }
  ...
  public function stopWords($text) {
    ...
    $statement = $con->prepare("SELECT token FROM stopWord WHERE `token` =  ?");
